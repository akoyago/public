...
if ($missingPluginTypes.Count -gt 0) {
    ...
    if ($targetAssemblyVersion -eq $exportedAssemblyVersion) {
        ...
        Write-Host "  WARNING: Missing plugin types with matching version indicates the assembly may have been manually modified" -ForegroundColor Yellow
        Write-Host "  RECOMMENDATION: This scenario requires manual re-registration of the plugin assembly" -ForegroundColor Yellow
        Write-Host "  " -ForegroundColor Yellow
        Write-Host "  Skipping automatic re-registration to avoid potential data loss" -ForegroundColor Yellow
        Write-Host "  Please manually re-register the AkoyaGo.Plugins assembly using XrmToolBox Plugin Registration tool" -ForegroundColor Yellow
    }
    else {
...
if ($missingPluginTypes.Count -gt 0 -and $targetAssemblyVersion -eq $exportedAssemblyVersion) {
    Write-Host "  Re-registration skipped: Manual intervention required" -ForegroundColor Yellow
}
Write-Host ""
